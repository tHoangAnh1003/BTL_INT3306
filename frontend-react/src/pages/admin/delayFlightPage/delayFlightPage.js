...
const DelayFlightPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!selectedFlight || isNaN(selectedFlight)) {
        setError("Chuyến bay không hợp lệ. Vui lòng chọn lại.");
        return;
      }
      const res = await fetch(`http://localhost:8081/api/flights/${Number(selectedFlight)}/delay`, {
        method: "PUT",
        headers: {
  ...
  onChange={e => {
    ...
    if (value === "") {
      ...
      setSelectedFlight(Number(value));
    }
  }}
  required
>
